...
const Metro = require('./metro_map')
const db = require('../../model/db/mysql/mysql')

// Error!!!
async function busMetroTransfer(version) {
  ...
  metroStations.forEach(station => {
    ...
    busStops.forEach(busStop => {
      ...
      if (distance < 100) {
        ...
        const metroWaitingTime = metroWaitingTimeMap[StationID]
    
        counter++
        values.push([mysqlMetroStoplId0, mysqlBusStopId, timePeriodId, walkTime + busWaitingTime, version, distance])
        values.push([mysqlMetroStoplId1, mysqlBusStopId, timePeriodId, walkTime + busWaitingTime, version, distance])
        values.push([mysqlBusStopId, mysqlMetroStoplId0, timePeriodId, walkTime + metroWaitingTime, version, distance])
        values.push([mysqlBusStopId, mysqlMetroStoplId1, timePeriodId, walkTime + metroWaitingTime, version, distance])
        if (!mysqlMetroStoplId0 || !mysqlMetroStoplId1 || !mysqlBusStopId || !timePeriodId || !walkTime || !busWaitingTime ||! distance) {
