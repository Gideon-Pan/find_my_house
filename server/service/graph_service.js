...
function makeOfficeToNearbyStopEdges(graph, officeLat, officeLng, period, maxWalkDistance) {
  ...
  for (let id of graph.getAllIds()) {
    ...
    if (!lat || !lng) continue
    const distToStation = getDistance({ latitude: lat, longitude: lng }, { latitude: officeLat, longitude: officeLng })

    if (distToStation < maxWalkDistance) {
      const edge = new Edge(START_POINT_ID, id, period, distToStation / WALK_VELOCITY)
      graph.addEdge(edge)
